...
extension GetFormattedRouteNameX on Route<dynamic> {
  String? getFormattedRouteName(BuildContext context) {
    return switch (settings.name) {
      ...
      BoothsRoute.name => context.localize.booths_title,
      SksMenuRoute.name => context.localize.sks_menu,
      SksFavouriteDishesRoute.name => context.localize.sks_favourite_dishes,
      ...
      AboutUsRoute.name => context.localize.about_us,
      DigitalGuideRoute.name => context.localize.digital_guide,
      DigitalGuideObjectRoute.name => context.localize.digital_guide,
      DigitalGuideLiftDetailRoute.name => context.localize.lift,
      MicronavigationDetailRoute.name => context.localize.micro_navigation,
      DoorRoute.name => context.localize.door,
      TransportationDetailRoute.name => context.localize.transport,
      StairsRoute.name => context.localize.stairs,
      StairwayRoute.name => context.localize.stairway,
      RailingsRoute.name => context.localize.railing,
      RampsRoute.name => context.localize.ramp,
      ...
      LodgeRoute.name => context.localize.lodge,
      InformationPointRoute.name => context.localize.information_point,
      DressingRoomRoute.name => context.localize.dressing_room,
      ParkingRoute.name => context.localize.parking,
      ...
      NewsfeedRoute.name => context.localize.news,
      NotificationsRoute.name => context.localize.notifications,
      CalendarRoute.name => context.localize.calendar,
      RadioLuzRoute.name => context.localize.radio_luz_title,
      ActivityDaysRoute.name => context.localize.activity_days_title,
      ActivityDaysStandDetailRoute.name => context.localize.activity_days_stand_details,
      _ => null,
    };
  }
}
