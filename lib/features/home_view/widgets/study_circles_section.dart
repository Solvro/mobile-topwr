...
class StudyCirclesSection extends StatelessWidget {
  const StudyCirclesSection({super.key, required this.onNavigate});
...
class _StudyCirclesList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return switch (state) {
      AsyncLoading() =>
      const Padding(
        padding: EdgeInsets.only(left: HomeScreenConfig.paddingMedium,
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return MediumLeftPadding(
                            child: value == null
                              ...
                              onClick: () => onNavigate(value[index]?.id ?? "")
                              ,
                            ),
                          );
                        })),
