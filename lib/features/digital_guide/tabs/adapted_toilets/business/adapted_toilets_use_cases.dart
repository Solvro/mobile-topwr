...
typedef LevelWithToilets = ({
  Level level,
  IList<AdaptedToilet> adaptedToilets,
});

@riverpod
Future<IList<LevelWithToilets>> getLevelsWithToiletsUseCase(
  Ref ref,
  ...
  final levelsWithToiletData = await Future.wait(
    levels.map((level) async {
      final adaptedToilets =
          await ref.watch(adaptedToiletsRepositoryProvider(level).future);
      return (
        level: level.level,
        adaptedToilets: adaptedToilets,
      );
    }),
  );
  return levelsWithToiletData
      .where(
        (levelWithToilets) => levelWithToilets.adaptedToilets.isNotEmpty,
      )
      .toIList();
}
