...
class InformationPointAccessibilityManager extends AccessibilityCommentsManager {
  ...
  IList<String> getCommentsListForHearingImpairment() {
    return IList([l10n.has_sound_transmitter(infoPoint.hasSoundTransmitter.toString().toLowerCase())]);
  }
  ...
  IList<String> getCommentsListForMotorImpairment() {
    return IList([
      if (infoPoint.areAccessBarriers) l10n.are_access_bariers,
      l10n.is_movement_space(infoPoint.isMovementSpace.toString().toLowerCase()),
      l10n.counter_heigth_text(infoPoint.counterHeight),
      l10n.is_space_under_counter(infoPoint.isSpaceUnderCounter.toString().toLowerCase()),
    ]);
  }
