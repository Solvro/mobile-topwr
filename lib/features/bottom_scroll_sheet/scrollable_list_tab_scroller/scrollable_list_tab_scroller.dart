...
class ScrollableListTabScrollerState extends State<ScrollableListTabScroller> {
  ...
  Future<void> _triggerScrollInPositionedListIfNeeded(int index) async {
    if (getDisplayedPositionFromList() != index &&
      ...
      if (itemScrollController.isAttached) {
        await itemScrollController.scrollTo(index: index, duration: widget.animationDuration);
      }
    }
  }
  ...
  Widget buildCustomBodyContainerOrDefault({required BuildContext context, required Widget child}) {
    return widget.bodyContainerBuilder?.call(context, child) ?? Expanded(child: child);
  }

  Future<void> _onScrollsToTop(ScrollsToTopEvent event) async {
    await itemScrollController.scrollTo(index: 0, duration: event.duration, curve: event.curve);
  }
      ...
      children: [
        buildCustomHeaderContainerOrDefault(
          ...
          child: DefaultHeaderWidget(
            ...
            onTapTab: _triggerScrollInPositionedListIfNeeded,
